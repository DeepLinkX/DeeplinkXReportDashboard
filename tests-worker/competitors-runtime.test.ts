  ...
  await env.DB.batch([
    env.DB.prepare(
      `INSERT INTO competitor_classifications (
        ...
        'capabilities-v2.4', 'https://pub.dev/packages/map_launcher', 'Launches installed maps.', 1, 2, 2, ?)`,
    ...
    env.DB.prepare(
      `INSERT INTO competitor_classifications (
        ...
        'capabilities-v2.4', 'https://pub.dev/packages/iconify_flutter_plus', 'Icon package.', 0, ?)`,
    ).bind("2026-09-02T00:00:00.000Z"),
  ]);
...
describe("competitor classification runtime", () => {
  ...
  it("resumes an interrupted backfill from its durable running record", async () => {
    ...
    expect(states.results).toEqual([{ status: "queued", classifier_version: "capabilities-v2.4", count: 2 }]);
  });
