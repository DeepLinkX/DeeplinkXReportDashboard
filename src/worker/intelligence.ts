...
export async function refreshPackage(env: Env, name: string, attempts = 1, runId?: string): Promise<IntelligencePackage> {
  ...
  if (likely && (!fresh(row.documentation_captured_at, 30) || row.documentation_version !== metadata.version)) {
    try {
...
export async function packageResponse(env: Env, name: string): Promise<unknown | null> {
  ...
  return { package: directoryPackage(row), discoveries: evidence.results, metrics: metrics.results.map((m) => ({ captured_at: m.captured_at, ...JSON.parse(m.metrics_json) })),
...
export async function startIntelligence(env: Env, key: string, runId?: string, full = true): Promise<unknown> {
  ...
  const condition = full
    ? "WHERE NOT EXISTS (SELECT 1 FROM competitor_classifications cc WHERE cc.package_name=cr.package_name AND cc.run_id=?)"
...
export async function importReview(env: Env, body: {package_name:string;evidence_hash:string;product_commit:string;reviewed_by:string;decision:PackageAnalysis}): Promise<void> {
  const row=await env.DB.prepare("SELECT evidence_hash,product_commit FROM competitor_registry WHERE package_name=?").bind(body.package_name).first<{evidence_hash:string;product_commit:string}>();
  if (!row || row.evidence_hash!==body.evidence_hash || row.product_commit!==body.product_commit) throw new Error("Review evidence is stale or unavailable.");
  ...
  const analysis={...decision,review_status:"reviewed"};const stamp=now();
  await env.DB.batch([
    env.DB.prepare("INSERT INTO competitor_reviews(package_name,evidence_hash,product_commit,decision_json,reviewed_by,reviewed_at) VALUES(?,?,?,?,?,?) ON CONFLICT(package_name,evidence_hash,product_commit) DO UPDATE SET decision_json=excluded.decision_json,reviewed_by=excluded.reviewed_by,reviewed_at=excluded.reviewed_at")
